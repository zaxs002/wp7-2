...
class YIT_Contact_Form {
    ...
    public function init() {
        ...
        add_action( 'init', array( $this, '_sendEmail' ) );
    ...
    public function _sendEmail()
    {
        ...
        if ( isset( $_POST['yit_action'] ) AND ( $_POST['yit_action'] == 'sendemail' OR $_POST['yit_action'] == 'sendemail-ajax' ) )
        {
            ...
            if( isset( $_POST[ 'yit_product_id' ] ) ) { $product = get_product( intval( $_POST[ 'yit_product_id' ] ) ); }
            ...
            {
                $subject = str_replace( $shortcode, $val, $subject );
    ...
    public function module( $name_or_id, $echo = true )
    {
        ...
        if( isset( $product ) ){ $html .= "\t\t\t\t" . '<input type="hidden" name="yit_product_id" value="' . $product->id .  '" id="yit_product_id" />' . "\n"; }
